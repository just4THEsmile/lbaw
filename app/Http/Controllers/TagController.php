...
{   
    ...
    if (strlen($query) == 0) {
        $results = Tag::limit(5)->get();//fix query to not return tags already in question
    } else {
        // Use where() with the 'like' operator to search usernames containing the query string
        $results = Tag::where('title','ILIKE',"%$query%" )->limit(5)->get();
    }

    return response()->json($results);
    }
    public function getTagsOfQuestion(Request $request)
    {
    $id = $request->input('id');
